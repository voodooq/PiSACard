  ...
  <div class="common_body">
    ...
    <div class="tc">
      <el-select v-model="seledCardPackage"
                 ...
                 :disabled="openList">
        <el-option v-for="item in cardPackage.filter(item =>{return item.starShopOpen})"
                   :key="item.packageId"
                   :label="item.name"
    ...
    <el-collapse-transition>
      <div class="wm_shop_item_list_body"
        ...
        <div class="wm_shop_item_list_box">
          <div class="wm_shop_item_list_ico"
               @click="buy(0,0)"></div>
          <div class="wm_shop_item_list_text">单抽<br />需要100星星</div>
        </div>
        <div class="wm_shop_item_list_box">
          <div class="wm_shop_item_list_ico"
               @click="buy(0,1)"></div>
          <div class="wm_shop_item_list_text">十连抽<br />需要900星星</div>
        </div>
        <div class="wm_shop_item_list_box">
          <div class="wm_shop_item_list_ico"
               @click="buy(0,2)"></div>
          <div class="wm_shop_item_list_text">三十连抽<br />需要2550星星</div>
        </div>
        <div class="wm_shop_item_list_box">
          <div class="wm_shop_item_list_ico"
               @click="buy(0,3)"></div>
          <div class="wm_shop_item_list_text">五十连抽(保底1张六星)<br />需要4000星星</div>
        </div>
        <div class="wm_shop_item_list_box">
          <div class="wm_shop_item_list_ico"
               @click="buy(0,4)"></div>
          <div class="wm_shop_item_list_text">一百连抽(保底2张六星)<br />需要7500星星</div>
        </div>
      </div>
    </el-collapse-transition>
    <menuView></menuView>
...
export default {
  ...
  methods: {
    ...
    getCardPackage () {
      authApi.searchcardpackage({ sortType: "starShopOpen" }).then(res => {
        console.log(res);
        if (res.data.code == 0) {
          ...
          // 给卡包排序
          this.cardPackage = packageSort(this.cardPackage, res.data.sortData, "starShopOpen");
          let nowPackageId = localStorage.getItem("shopCardPackageId");
          this.seledCardPackage = "";
          // 如果缓存中存在默认卡牌ID
          if (nowPackageId) {
            const seledCardPackageInfo = this.cardPackage.find((item) => {
              return item.packageId === nowPackageId && item.starShopOpen;
            });
